 ...
 * 我们可以这么认为:String是接收者类型,this则是具体的接收者类型对象,当需要访问接受者对象的属性/方法时,也只是通过外部访问的方式去访问的。
 * 具体的原理可以看编译生成的java代码.编译生成的java静态方法大概是这样的：
 * public static Boolean cxIsNotEmpty(String obj){ obj.isNotEmpty}
 * 编译生成的拓展函数方法特点如下：
 * (1)拓展函数一般在顶层中声明,因此一定是public static修饰的
 * (2)方法参数的第一个参数一定是要求传入一个接受者类型的对象,其他参数其后
 * (3)拓展函数内部访问接受者对象的属性或方法，其实都是通过外部访问的方式(即对象.属性/对象.方法名)来访问的。因此，
 * 在拓展函数中,私有的属性和私有的方法,是没办法访问的,毕竟本质是外部访问。
 * (4)根据生成的java代码可知:拓展函数是不能被子类重写的，因为它是静态方法不属于对象
 * (5)对父类进行的拓展子类是肯定能够使用的.因为第一个参数是接受者类型对象,其子类肯定能传入。
 * [父类的拓展函数,子类是可以使用的]---->意味着例如A,B两个类,A是B的父类,则我们对A类进行拓展之后,B类也可以使用父类A的拓展函数。
 * (6)拓展函数强调对类进行函数拓展,
 * 因此List<String>.test();和 List<Int>.test();是等价的。声明了List<String>.test();就不能写 List<Int>.test(); 因为会被认为重复声明了相同的拓展函数。
 *
